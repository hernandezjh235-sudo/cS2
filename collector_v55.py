"""OneWayPickz CS2 v5.9.2 collector entrypoint.
...
classification, complete live-line visibility, direct HLTV provider/map/profile data,
...
def export_provider_bridge(ns: dict, board: list[dict], previous: dict | None = None) -> dict:
    ...
    for key, rec in list(profiles.items()):
        ...
        if team:
            ...
            merged["identity_verified_source"] = dbrec.get("identity_verified_source") or "v5.9.2 collector"
    ...
    bridge["schema_version"] = max(16, int(bridge.get("schema_version") or 0))
    bridge["profiles"] = profiles
    bridge["teams"] = teams
    status = dict(bridge.get("source_status") or {})
    status.update({
        "autofeed_version":"5.9.2",
    ...
    try:
        ...
        provider_health = ns["load_json"](ns.get("V590_PROVIDER_HEALTH_FILE"), {}) if ns.get("V590_PROVIDER_HEALTH_FILE") else {}
        hltv_health = ns["load_json"](ns.get("V591_HEALTH_FILE"), {}) if ns.get("V591_HEALTH_FILE") else {}
        profile_health = ns["load_json"](ns.get("V592_PROFILE_HEALTH_FILE"), {}) if ns.get("V592_PROFILE_HEALTH_FILE") else {}
        ...
        if isinstance(readiness, dict):
            status["readiness_version"] = readiness.get("version"); status["missing_projection_requirements"] = dict(readiness.get("missing_projection_requirements") or {})
        if isinstance(provider_health, dict): status["provider_discovery_version"] = provider_health.get("version"); status["provider_discovery"] = dict(provider_health.get("discovery") or {}); status["provider_index"] = dict(provider_health.get("index") or {})
        if isinstance(hltv_health, dict): status["hltv_direct_version"] = hltv_health.get("version"); status["hltv_direct_health"] = hltv_health
        if isinstance(profile_health, dict): status["hltv_profile_version"] = profile_health.get("version"); status["hltv_profile_health"] = profile_health
    except Exception: pass
    ...
    if callable(seed):
        try:
            bridge["source_status"]["database_seed_v592"] = seed(bridge); ns["save_json"](str(path), bridge, force=True)
        except Exception as exc: bridge["source_status"]["database_seed_v592_warning"] = str(exc)
    return bridge
...
def _force_fresh_cycle() -> None:
    ...
    try: (data_dir / ".autofeed_collector.heartbeat").unlink(missing_ok=True)
    except Exception: pass
