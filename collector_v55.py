"""OneWayPickz CS2 v5.9.1 collector entrypoint.
...
def export_provider_bridge(ns: dict, board: list[dict], previous: dict | None = None) -> dict:
    ...
    for key, rec in list(profiles.items()):
        ...
        if team:
            ...
            merged["identity_verified_source"] = dbrec.get("identity_verified_source") or "v5.9.1 collector"
    ...
    bridge["schema_version"] = max(15, int(bridge.get("schema_version") or 0))
    bridge["profiles"] = profiles
    bridge["teams"] = teams
    status = dict(bridge.get("source_status") or {})
    status.update({
        "autofeed_version": "5.9.1",
    ...
    try:
        ...
        if isinstance(provider_health, dict):
            status["provider_discovery_version"] = provider_health.get("version")
            status["provider_discovery"] = dict(provider_health.get("discovery") or {})
            status["provider_index"] = dict(provider_health.get("index") or {})
        if isinstance(hltv_health, dict):
            status["hltv_direct_version"] = hltv_health.get("version")
            status["hltv_direct_health"] = hltv_health
    except Exception:
        pass
    ...
    if callable(seed):
        try:
            bridge["source_status"]["database_seed_v591"] = seed(bridge)
            ns["save_json"](str(path), bridge, force=True)
        except Exception as exc:
            bridge["source_status"]["database_seed_v591_warning"] = str(exc)
    return bridge
